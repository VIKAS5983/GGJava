...
public class LevelOrderAndInorder {
	    ...
	    static TreeNode buildTree(int in[], int level[], int inStrt, int inEnd) 
	    { 
	        if (inStrt > inEnd) 
	            return null; 
	      ...
	      //  System.out.println("isStr: inEnd "+inStrt +" : "+inEnd);
	        TreeNode tNode = new TreeNode(level[levelIndex++]); 
	        ...
	        /* Using index in Inorder traversal, construct left and 
	           right subtress */
	        tNode.left = buildTree(in, level, inStrt, inIndex - 1); 
	        tNode.right = buildTree(in, level, inIndex + 1, inEnd); 
